...
```
이슈 작성 (담당자 · 브랜치 이름 · 수정 허용 파일)   ← 팀장. back 영역은 이재원도
  → 담당자가 이슈의 브랜치에서 작업
...
| 할 일 | 봉준표 (팀장) | 이재원 | 조수희 |
|---|---|---|---|
| 작업 이슈 작성 | 전 영역 | **`back` 영역만** — 자신에게 배정 | 하지 않는다 |
| 버그 · 질문 · 요청 이슈 (빈 이슈) | 한다 | 한다 — 담당자 배정은 팀장 | 한다 — 담당자 배정은 팀장 |
| 작업 시작 | 이슈를 만든 뒤 | **자기가 쓴 이슈면 팀장 확인 없이 시작** | 팀장이 올린 이슈로만 |
| 브랜치 이름 | 짓는다 | **짓는다** — `<종류>/back-<설명>` | 이슈에 적힌 이름 그대로 |
| 취소 · 중복 이슈 닫기 + 원격 브랜치 삭제 | 전부 | **자기가 쓴 `back` 이슈만** | 하지 않는다 |
| 계약(`CONTRACT_API.md`) 변경 | 재원과 같은 PR | 준표와 같은 PR | 하지 않는다 |
| **PR 머지** | **단독** | **하지 않는다** — 자기가 쓴 이슈라도 | 하지 않는다 |

이재원 권한은 2026-09-17 준표 결정(A안)이다. **이 표에 없는 권한은 없다.**
...
**브랜치 이름은 이슈를 쓰는 사람이 짓고, 담당자는 이슈에 적힌 이름을 그대로 쓴다.** 조수희는 직접 짓지 않는다.

### 수명은 3일

**3일보다 오래 사는 브랜치는 자른 게 너무 큰 것이다.** 이슈를 쪼갠다 — 이재원은 자기 이슈를 직접 쪼개고, 조수희는 팀장에게 요청한다.
...
- **PR 없이 이슈를 닫는 경우**(취소 · 중복)는 **이슈를 닫는 사람이** 사유를 댓글로 남기고 원격 브랜치를 함께 지운다 — 팀장, 또는 자기가 쓴 `back` 이슈를 닫는 이재원. **남의 브랜치는 지우지 않는다**

```
git push origin --delete feat/back-uv-index-api    # 자기 브랜치만. main은 훅이 거부한다
```
...
## 한 번에 여는 PR은 하나 (2026-09-20 준표 결정)

**봉준표의 작업은 PR을 하나씩 연다.** 머지된 뒤에 다음 이슈를 시작한다.

PR 여러 개가 **같은 파일 끝에 각자 줄을 더하면**, 하나를 머지할 때마다 나머지가 다시 충돌한다. PR이 3개면 2번, 4개면 3번 난다. 실제로 #38이 두 번 겪었다.

조수희·이재원은 **영역이 달라** 동시에 열어도 된다. 같은 파일을 건드릴 것 같으면 이슈 댓글로 먼저 맞춘다.

### 자주 부딪히는 파일

| 파일 | 왜 |
|---|---|
| [`design/screens/README.md`](design/screens/README.md) | 캡처 목록 표. 모두 맨 끝에 줄을 더한다 |
| [`../frontend/README.md`](../frontend/README.md) | 페이지 표 · 폴더 담당 표 |
| `frontend/lib/types.ts` | 새 타입을 파일 끝에 더한다 |
...
git switch <내 브랜치>
git fetch origin
git merge origin/main        # rebase 는 쓰지 않는다
# 표에 줄이 두 개 생긴 자리는 둘 다 남긴다 — 한쪽이 틀린 게 아니다
git add <푼 파일>
git commit
git push
```

**`<<<<<<<` `=======` `>>>>>>>` 마커를 남기지 않는다.** 푼 뒤에 테스트를 다시 돌린다.
