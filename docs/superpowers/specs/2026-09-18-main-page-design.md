...
히어로 길이: PC `255svh`, 모바일(≤700px) **`300svh`**. (2026-09-22 정정 — 195svh 는 실제 기기에서 스크롤 여유가 450px 뿐이라 한 번 쓸어내리면 연출이 끝났다. 300svh 면 여유가 1300px 안팎이 된다.) 스크롤 컨테이너에 `overflow-anchor: none`을 준다 — 없으면 sticky 구간에서 스크롤이 되돌아간다.
